...
dfXcp_r = pd.DataFrame(predict_cluster_label)
dfXcp_r.to_csv("load_r.csv",index=False)

X_train, X_test, y_train, y_test = train_test_split(X,predict_cluster_label, test_size=0.33, random_state=10)
print(X_train.shape)
print(y_train.shape)
print(X_test.shape)
print(y_test.shape)
clf = RandomForestClassifier(n_estimators=100, criterion="gini", random_state=10)
...
dfXcp.to_csv("no_load_rr.csv",index=False)
